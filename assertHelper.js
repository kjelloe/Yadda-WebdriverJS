...
module.exports = {
  init: function (providedDriver) {
	...
	// Get named attribute
	helper.getAttribute = function(element, attributeName) {
		...
		element.getAttribute(attributeName).then(function(attribValue) {
			...
			if(attributeName.toLowerCase()==='innertext' && attribValue==null) {
				helper.getTextContent(element).then( function(textValue) {
	...
	// Get text content from DOM object
	helper.getTextContent = function(element) {
		var elemDone = webdriver.promise.defer();
    ...
    helper.loadUrl = function(url, forceRefresh) {
      var loadDone = webdriver.promise.defer();
      helper.driver.get(url).then( function(page) {
		helper.driver.sleep(100); // Wait for load
        ...
        helper.driver.findElement(webdriver.By.tagName('body')).getTagName().then(function(tagname) {
			...
			if(forceRefresh===true) {
				helper.driver.navigate().refresh().then( function() {
					helper.elementWaitVisible('body');
